...
beforeAll(() => {
  console.log("모든 테스트가 실행하기 전에 한번만 실행된다.");
});
afterAll(() => {
...
describe("음수 양수 판단", () => {
  test("isNegtive에 7을 넣으면 false를 반환한다.", () => {
    const calculator = new Calculator();
    expect(calculator.isNegative(7)).toBeFalsy();
  });
  test("isNegtive에 -3을 넣으면 true를 반환한다.", () => {
    const calculator = new Calculator();
    expect(calculator.isNegative(-3)).toBeTruthy();
  });
  test("isNegtive에 0을 넣으면 true가 아니다.", () => {
    const calculator = new Calculator();
    expect(expect(calculator.isNegative(0)).toBeFalsy()).not.toBe('true');
  });
});
